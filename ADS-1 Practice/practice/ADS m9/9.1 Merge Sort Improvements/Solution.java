...
// }
class Mergesort {
    void merge(String[] inputarray, int first, int middle, int last) {
        int i = first;
        int j = middle + 1;
        String[] test = Arrays.copyOf(inputarray, last - first + 1);
        System.out.println(Arrays.toString(test) + "entered into merge");
    }
    void sort(String[] inputarray, int first, int last) {
        InsertionSort insertion = new InsertionSort();
        if(last - first <= 7) {
        ...
        int middle = (first + last)/2;
        sort(inputarray, first, middle);
        sort(inputarray, middle + 1, last);
        merge(inputarray, first, middle, last);
        // String[] storingarray = Arrays.copyOf(inputarray);
    }
}
/**
 * Class for solution.
